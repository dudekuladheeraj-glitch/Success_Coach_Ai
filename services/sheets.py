...
# ) -> None:
#     """
#     Append one new row to the 'signal_sheet' tab after a session ends (M6).
#     Columns: student_id | signal_type | severity | urgency | reason | timestamp | actioned

#     actioned is always written as FALSE for a new signal — it's the coach's
#     job (or a future M9 automation) to mark it actioned once handled.
#     """
#     worksheet = _worksheet("signal_sheet")
#     timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#     worksheet.append_row(
#         [student_id, signal_type, severity, urgency, reason, timestamp, "FALSE"],
...
#     """
#     All un-actioned signals across the ENTIRE roster — not filtered to one
#     student. Used for the coach-facing alert panel (M6) so high/critical
#     signals are visible regardless of which student is currently selected.

#     Returns list of:
#       {student_id, signal_type, severity, urgency, reason, timestamp}
#     sorted newest first.
#     """
#     rows = _worksheet("signal_sheet").get_all_records()
#     parsed = []
#     for row in rows:
#         actioned_raw = str(row.get("actioned", "")).strip().lower()
#         actioned = actioned_raw in ("true", "yes", "1", "y")
