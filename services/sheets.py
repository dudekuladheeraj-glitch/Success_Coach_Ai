...
#         # ── Service account (recommended — no browser pop-up) ──────────────
...
#                     print(f"[sheets] Removed malformed {token_path} — will re-authenticate.")
#             except Exception:
#                 os.remove(token_path)
#                 print(f"[sheets] Removed unreadable {token_path} — will re-authenticate.")
...
#         raise ValueError(
#             "GOOGLE_SPREADSHEET_ID is not set in .env\n"
...
def get_exam_scores(student_id: str) -> list[dict]:
    ...
    # Sort chronologically; rows with unparseable dates go last in original order
